...
<?php foreach($this->items as $i => $item): 
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td>
				<a href="<?php echo JRoute::_('index.php?option=com_mpoll&task=answer.edit&q_id='.(int) $item->q_id); ?>">
		...
		<?php 
			if ($item->q_type=='multi' || $item->q_type=='mcbox' || $item->q_type=='dropdown') {
				echo '<a href="'.JRoute::_('index.php?option=com_continued&view=answers&opt_question='.$item->q_id).'">Answers'; 
				$db =& JFactory::getDBO();
