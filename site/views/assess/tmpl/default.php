...
$numcorrect = 0;
$totq = 0;
if ($this->cinfo['course_evaltype'] == 'assess' && $this->cinfo['haseval']) {
	foreach ($this->qanda as $qanda) {
		$assess .= '<tr><td align="left">';
		$assess .= $qanda['ordering'].'. <b>'.$qanda['qtext'];
		$assess .= '</b></td><td align="right"><b>';
		if ($qanda['qtype'] == 'multi') {
			if ($qanda['qcat'] == 'assess' && $qanda['correct'] == 1) {
				$assess .='<span style="color:#008000">'.$qanda['opttxt'].'</span></b></td><td align="right"><b><span style="color:#008000">Correct'; $numcorrect++;
			}
			if ($qanda['qcat'] == 'assess' && $qanda['correct'] == 0) {
				$assess .= '<span style="color:#800000">'.$qanda['opttxt'].'</span></b></td><td align="right"><b><span style="color:#800000">Incorrect';
			}
			if ($qanda['qcat'] == 'eval') $assess .= $qanda['opttxt'].'</b></td><td><b><span>';
			$totq++;
			$assess .= '</span>';
		}
		...
		$assess .= '</b></td></tr>';
		if ($cecfg['EVAL_EXPL']) {
			if ($qanda['optexpl']) $assess .= '<tr><td colspan="3">'.$qanda['optexpl'].'</td></tr>';
...
if ($pass) {
	...
	else echo '<p align="center">Thank you for completing: '.$this->cinfo['cname'].'.<br /><br/>';
	if ($this->cinfo['hascertif']) echo 'Select the Get Certificate button to receive your '.$this->usercertif.'.<br />';
	echo 'Select the return button below to return to the list.</p>';
	echo '<p align="center">';
	if (!$this->cando && $this->cinfo['hascertif']) {
	...
	echo '<a href="'.$this->cinfo['cataloglink'].'"><img src="media/com_continued/template/'.$cecfg['TEMPLATE'].'/btn_return.png" border="0" alt="Return"></a></p>';
}
else
{
	...
	echo '<a href="'.$this->cinfo['cataloglink'].'"><img src="media/com_continued/template/'.$cecfg['TEMPLATE'].'/btn_return.png" border="0" alt="Return"></a>';
}
?>
